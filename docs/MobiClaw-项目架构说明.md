# MobiClaw 项目架构说明（按当前实际代码口径）

## 1. 当前定位

MobiClaw 当前实际是一个以 **多 Agent 编排** 为核心的执行层，负责把：
...
> MobiClaw 当前是一套以 `Gateway / Chat + Orchestrator + Agents + MobiAgent + 本地状态` 为核心的多 Agent 编排系统。
...
│   ├── gateway_server.py           # MobiClaw 对外 HTTP Gateway
...
核心文件：`mobiclaw/orchestrator.py` + `mobiclaw/agents.py`
...
`mobiclaw/gateway_server.py` 当前承担完整任务网关能力。
...
MobiClaw 当前不是“多 Agent + 多网关 + 知识库”的三层架构，而是：
