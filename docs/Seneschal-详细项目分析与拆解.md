...
- WeKnora 相关代码仍然存在
- 但 WeKnora **已经不是当前主链路的真实依赖**
- 文档中不应再继续把 WeKnora 写成现行核心模块
...
当前仓库中仍然能看到大量 WeKnora 痕迹：

- `seneschal/tools/weknora/`
- `seneschal/tools/__init__.py` 中的 `weknora_*` 高层封装
- `seneschal/config.py` 中的 `WEKNORA_*`
- `seneschal/gateway_server.py` 中的 WeKnora 配置表单项
- `seneschal/dailytasks/runner.py` 中的 `weknora_add_knowledge` / `weknora_rag_chat`
- 旧文档中大量以 WeKnora 为中心的描述

### 2.2 当前为什么不能把它当主架构

从当前真实执行链路看：

- Gateway 主流程不会强依赖 WeKnora
- Chat 主流程不会强依赖 WeKnora
- Orchestrator 的 route/plan/execute 主干不会强依赖 WeKnora
- Worker 的主要执行能力来自 web/papers/shell/office/ppt/ocr/memory
- Steward 的核心闭环依赖的是 MobiAgent 返回的执行证据，而不是 WeKnora

所以更准确的说法是：

> WeKnora 现在是 **legacy / 兼容残留**，不是现行核心模块。
...
> Daily 当前仍然存在，但其内部还没有完全同步到新的“本地状态 + 多 Agent 主链路”口径。
...
### 11.1 还把 WeKnora 写成主架构核心

这是最显著的历史遗留。

### 11.2 还把 `--agent-task` 说成直接 Worker

当前默认已经是 orchestrator 主链路。

### 11.3 低估了 Gateway 和 Chat 的产品化程度

它们现在已经不是辅助模块，而是核心入口。

### 11.4 低估了 Orchestrator 的中心地位
