...
> **MobiClaw 是一套以 Gateway/Chat 为入口、以 Orchestrator + Agents 为核心、以 MobiAgent 和本地工具/本地状态为基础的多 Agent 编排系统。**
...
- `mobiclaw/orchestrator.py: run_orchestrated_task()`
...
`mobiclaw/gateway_server.py` 当前已经不仅仅是转发器，而是：
...
`mobiclaw/dailytasks/runner.py` 当前仍然是一个混合态模块。
...
### 8.1 还把 `--agent-task` 说成直接 Worker

当前默认已经是 orchestrator 主链路。

### 8.2 低估了 Gateway 和 Chat 的产品化程度

它们现在已经不是辅助模块，而是核心入口。

### 8.3 低估了 Orchestrator 的中心地位
...
> MobiClaw 当前已经从“以知识库为中心的编排设想”，演化成了“以 Gateway/Chat + Orchestrator + MobiAgent + 本地状态为中心的多 Agent 执行系统”。
