...
依赖模块：
- `mobiclaw.agents`：创建用户/执行 Agent；
- `mobiclaw.orchestrator`：多 Agent 智能路由与执行；
- `mobiclaw.dailytasks.runner`：定时/触发型每日任务流程。
"""
...
async def run_interactive_mode() -> None:
    """运行交互式对话模式。"""
    print("=" * 70)
    print("MobiClaw 个人数据管家智能体系统 - 交互模式")
    print("=" * 70)
    print()
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="MobiClaw workflow entrypoint")
    parser.add_argument(
        "--interactive",
        action="store_true",
