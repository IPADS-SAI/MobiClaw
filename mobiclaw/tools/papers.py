...
async def arxiv_search(
    ...
    timeout_s = float(os.environ.get("MOBICLAW_WEB_TIMEOUT", "15"))
...
async def dblp_conference_search(
    ...
    headers = {
        "User-Agent": "MobiClaw/0.1 (+https://github.com/)",
...
async def download_file(
    ...
    if error == "path_outside_root":
        return ToolResponse(
            content=[TextBlock(type="text", text="[Download] Path is outside MOBICLAW_FILE_WRITE_ROOT.")],
            metadata={"error": error},
        )

    timeout_s = float(os.environ.get("MOBICLAW_WEB_TIMEOUT", "15"))
    limit_env = os.environ.get("MOBICLAW_DOWNLOAD_MAX_BYTES", "50000000")
...
async def extract_pdf_text(
    ...
    max_chars_env = os.environ.get("MOBICLAW_PDF_MAX_CHARS", "12000")
