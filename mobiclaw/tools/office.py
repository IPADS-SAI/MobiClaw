...
def _pick_reportlab_font() -> tuple[str, str]:
    ...
    configured_pdf = (os.environ.get("MOBICLAW_PDF_FONT_PATH") or "").strip()
    configured_cjk = (os.environ.get("MOBICLAW_CJK_FONT_PATH") or "").strip()
    candidates: list[str] = []
    if configured_pdf:
...
async def read_docx_text(
    ...
    max_chars_env = os.environ.get("MOBICLAW_DOCX_MAX_CHARS", "12000")
...
async def create_docx_from_text(
    ...
    if error == "path_outside_root":
        return ToolResponse(
            content=[TextBlock(type="text", text="[DOCX] Path is outside MOBICLAW_FILE_WRITE_ROOT.")],
            metadata={"error": error},
        )
...
async def create_pdf_from_text(
    ...
    if error == "path_outside_root":
        return ToolResponse(
            content=[TextBlock(type="text", text="[PDF] Path is outside MOBICLAW_FILE_WRITE_ROOT.")],
            metadata={"error": error},
        )
    ...
    try:
        ...
        if font_source == "builtin":
            logger.warning(
                "office.pdf.no_cjk_font_found; set MOBICLAW_PDF_FONT_PATH or MOBICLAW_CJK_FONT_PATH to a valid CJK font file to avoid garbled Chinese text"
            )
...
async def write_xlsx_from_records(
    ...
    if error == "path_outside_root":
        return ToolResponse(
            content=[TextBlock(type="text", text="[XLSX] Path is outside MOBICLAW_FILE_WRITE_ROOT.")],
            metadata={"error": error},
        )
