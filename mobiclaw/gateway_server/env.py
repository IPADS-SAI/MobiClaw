...
_ENV_SETTINGS_SCHEMA: list[dict[str, Any]] = [
    {
        ...
        "title": "Runtime",
        "items": [
            {"key": "MOBICLAW_LOG_LEVEL", "label": "日志级别", "type": "select", "options": ["DEBUG", "INFO", "WARNING", "ERROR"]},
            {"key": "MOBICLAW_FILE_WRITE_ROOT", "label": "文件输出根目录", "type": "text"},
            {"key": "MOBICLAW_CHAT_SESSION_ROOT", "label": "Chat 会话目录", "type": "text"},
        ],
    },
    {
        "id": "gateway",
        "title": "Gateway",
        "items": [
            {"key": "MOBICLAW_GATEWAY_HOST", "label": "监听主机", "type": "text"},
            {"key": "MOBICLAW_GATEWAY_PORT", "label": "监听端口", "type": "number"},
            {"key": "MOBICLAW_GATEWAY_API_KEY", "label": "API Key", "type": "password"},
            {"key": "MOBICLAW_GATEWAY_PUBLIC_BASE_URL", "label": "公网访问地址", "type": "text"},
            {"key": "MOBICLAW_GATEWAY_FILE_ROOT", "label": "可下载文件根目录", "type": "text"},
            {"key": "MOBICLAW_GATEWAY_CALLBACK_TIMEOUT", "label": "回调超时(s)", "type": "number"},
            {"key": "MOBICLAW_GATEWAY_CALLBACK_RETRY", "label": "回调重试次数", "type": "number"},
            {"key": "MOBICLAW_GATEWAY_CALLBACK_BACKOFF", "label": "回调退避(s)", "type": "number"},
        ],
    },
    {
        "id": "llm",
    ...
    },
    {
        ...
        "title": "Routing",
        "items": [
            {"key": "MOBICLAW_ROUTING_DEFAULT_MODE", "label": "默认模式", "type": "select", "options": ["chat", "router", "intelligent", "worker", "steward", "auto"]},
            {"key": "MOBICLAW_ROUTING_STRATEGY", "label": "路由策略", "type": "text"},
            {"key": "MOBICLAW_ALLOW_LEGACY_MODE", "label": "允许 legacy 模式(0/1)", "type": "text"},
            {"key": "MOBICLAW_ROUTING_MAX_SUBTASKS", "label": "最大子任务数", "type": "number"},
            ...
            {"key": "MOBICLAW_WORKER_TOOL_TIMEOUT_S", "label": "Worker Agent 工具调用超时(s)", "type": "number"},
            {"key": "MOBICLAW_STEWARD_TOOL_TIMEOUT_S", "label": "Steward Agent 工具调用超时(s)", "type": "number"},
            {"key": "MOBICLAW_CUSTOM_TOOL_TIMEOUT_S", "label": "Custom Agent 工具调用超时(s)", "type": "number"},
            {"key": "MOBICLAW_CHAT_TOOL_TIMEOUT_S", "label": "Chat Agent 工具调用超时(s)", "type": "number"},
...
def _render_structured_env_content(values: dict[str, str], unmanaged: dict[str, str]) -> str:
    ...
    lines: list[str] = [
        "# Auto-generated by MobiClaw Gateway Console",
